...
function ForgotPasswordScreen({ setLoginView, mockUsers }) {
    ...
    const handleAnswerSubmit = async (e) => {
        ...
        if (user && answer1.toLowerCase() === user.securityAnswer.toLowerCase()) {
            setStep(3);
        } else {
            setError('Incorrect answer. Please contact an administrator to recover your account.');
        }
    }
    ...
    const handleFinalSubmit = async (e) => {
        ...
        if (user && answer2.toLowerCase() === user.securityAnswer2.toLowerCase()) {
            setSubmitted(true);
        } else {
            setError('Incorrect answer. Please contact an administrator to recover your account.');
        }
                    ...
                    <>
                        ...
                        {step === 1 && (
                            <form onSubmit={handleUserSubmit}>
                                ...
                                <div className="mb-4">
                                    ...
                                    <input value={username} onChange={e => setUsername(e.target.value)} required className="w-full px-4 py-2 border rounded-lg"/>
                                </div>
                        ...
                        {step === 2 && user && (
                            <form onSubmit={handleAnswerSubmit}>
                                ...
                                <div className="mb-4">
                                    ...
                                    <input type="text" value={answer1} onChange={e => setAnswer1(e.target.value)} required className="w-full px-4 py-2 border rounded-lg"/>
                                </div>
                                {error && <p className="text-red-500 text-sm mb-4 text-center">{error}</p>}
                                <button type="submit" disabled={isLoading} className="w-full mt-4 bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2">
                                    {isLoading && <IconLoading className="w-5 h-5" />}
                        ...
                        {step === 3 && user && (
                            <form onSubmit={handleFinalSubmit}>
                                <p className="text-center text-gray-500 mb-6">Answer your second security question to proceed.</p>
                                <div className="mb-4">
                                    <label className="block text-gray-600 mb-2 font-semibold">{user.securityQuestion2}</label>
                                    <input type="text" value={answer2} onChange={e => setAnswer2(e.target.value)} required className="w-full px-4 py-2 border rounded-lg"/>
                                </div>
                                {error && <p className="text-red-500 text-sm mb-4 text-center">{error}</p>}
                                <div className="mb-4">
