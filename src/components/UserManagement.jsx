...
import { IconLoading, IconMail, IconPlusCircle } from './Icons';
...
function UserManagement({ mockUsers, updateUserInApp }) {
    ...
    const updateUser = (updatedUser) => {
        setUsers(prevUsers => ({
            ...prevUsers,
            [updatedUser.username]: { ...prevUsers[updatedUser.username], ...updatedUser }
        }));
        updateUserInApp(updatedUser);
    };

    const suspendUser = (suspendedUser) => {
        setUsers(prevUsers => {
            const updatedUsers = { ...prevUsers };
            updatedUsers[suspendedUser.username] = {
                ...updatedUsers[suspendedUser.username],
                status: 'Suspended',
                suspendUntil: suspendedUser.suspendUntil
            };
            return updatedUsers;
        });
    };
    ...
    const SuspendUserForm = ({ user, close }) => {
        ...
        const handleSubmit = async (e) => {
            ...
            suspendUser({ username: user.username, suspendUntil: suspendDate });
        ...
        return (
            <form onSubmit={handleSubmit}>
                ...
                {showNotification && (
                    <div className="absolute inset-x-0 bottom-0 m-4 p-4 rounded-lg bg-red-500 text-white text-center shadow-lg">
                        {user.fullName} suspended until {suspendDate}!
                    </div>
                )}
            </form>
        );
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-4">
                ...
                <div className="flex items-center space-x-2">
                    ...
                    <button onClick={() => openModal('create')} className="bg-teal-600 text-white py-2 px-4 rounded-lg hover:bg-teal-700 flex items-center space-x-2"><IconPlusCircle className="w-5 h-5" /><span>Create User</span></button>
                </div>
            </div>
                        ...
                        {filteredUsers.map(([username, user]) => (
                            <tr key={username} className="border-b">
                                ...
                                <td className="p-3 flex space-x-2 items-center">
                                    <button onClick={() => openModal('edit', { username, ...user })} className="text-blue-600 hover:underline text-sm">Edit</button>
                                    <button onClick={() => openModal('suspend', { username, ...user })} className="text-red-600 hover:underline text-sm">Suspend</button>
                                    <button onClick={() => openModal('email', { username, ...user })} className="text-gray-600 hover:text-black"><IconMail className="w-5 h-5" /></button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
