                    ...
                    <>
                        ...
                        {step === 1 && (
                            <form onSubmit={handleUserSubmit}>
                                ...
                                <button type="submit" disabled={isLoading} title="Verify your identity" className="w-full mt-4 bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2">{isLoading && <IconLoading className="w-5 h-5" />}<span>Continue</span></button>
                        ...
                        {step === 2 && user && (
                            <form onSubmit={handleAnswerSubmit}>
                                ...
                                {error && <p className="text-red-500 text-sm mb-4 text-center">{error}</p>}
                                <button type="submit" disabled={isLoading} title="Verify your answers" className="w-full mt-4 bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2">{isLoading && <IconLoading className="w-5 h-5" />}<span>Continue</span></button>
                                <button type="button" onClick={() => setLoginView('login')} title="Cancel and return to the login screen" className="w-full mt-2 text-center text-gray-500 hover:underline">Back to Login</button>
                            </form>
                        )}
                        {step === 3 && user && (
                            <form onSubmit={handleFinalSubmit}>
                                ...
                                {error && <p className="text-red-500 text-sm mb-4 text-center">{error}</p>}
                                <button type="submit" disabled={isLoading} title="Set your new password" className="w-full mt-4 bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2">{isLoading && <IconLoading className="w-5 h-5" />}<span>Reset Password</span></button>
                                <button type="button" onClick={() => setLoginView('login')} title="Cancel and return to the login screen" className="w-full mt-2 text-center text-gray-500 hover:underline">Back to Login</button>
                            </form>
                        )}
                    </>
